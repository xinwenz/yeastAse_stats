...
for(i in 1:nrow(cal)) {
  ...
  ck1 <- sum(startsWith(as.character(cal[i,yaN]), alt_ans)) 
  ...
  ck2 <- sum(startsWith(as.character(cal[i,raN]),alt_ans))
  
  evd_rcd_yA <- c(evd_rcd_yA,ck1)
  evd_rcd_rA <- c(evd_rcd_rA,ck2)
} 

wrong_rows <- which(evd_rcd_yA < 30 | is.na(evd_rcd_yA) | evd_rcd_rA < 30 | is.na(evd_rcd_rA) ) 
...
y <- colSums(cal[-wrong_rows,grep("^y.*[HA]_ac",names(cal),value=T)],na.rm=T)
xy <- data.frame(x,y)
ggplot(xy,aes(x=x,y=y)) + geom_point() + labs(title=" Total Read counts in 20 hybrid samples from YPS view" , x = "total read counts mapped to Yps128 allele" , y = "total read counts mapped to Rm11-1a allele" ) + scale_y_continuous(labels = scales::scientific) + scale_x_continuous(labels = scales::scientific) + geom_abline(slope = 1,intercept = 0) + theme(text=element_text(size=15))
