...
for(i in 1:500) {
    mydataName <- paste0("~/cloud/project/R2_simuP1_liko/simu_p1_liko_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_simuP1 <- fpo_format(nm="simu_p1_liko_",each_nr = 25,nr = 20, nGenes= 2000 , nbit=3)
...
ggplot(df_simuP1, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="R_2: simuP1_likelihood_ratio_sig0.05", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
