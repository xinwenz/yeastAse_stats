...
for(i in 1:500) {
    mydataName <- paste0("~/cloud/project/R3_simuP4_liko/simu_p4_liko_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_simuP4 <- fpo_format(nm="simu_p4_liko_",each_nr = 25,nr = 20, nGenes= 2000 , nbit=3)
...
ggplot(df_simuP4, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="R_3: simuP4_likelihood_ratio_sig0.05", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
