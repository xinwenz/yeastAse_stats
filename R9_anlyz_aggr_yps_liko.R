...
for(i in 1:500) {
    mydataName <- paste0("~/cloud/project/R9_cal_yps_liko/cal_agrg_yps_liko_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_cal_aggr_notail <- fpo_format(nm="cal_agrg_yps_liko_",each_nr = 25,nr = 20, nGenes= 952,nbit=3)
...
ggplot(df_cal_aggr_notail, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="Xinw2018_yps: false positive rate with different levels of replication", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
