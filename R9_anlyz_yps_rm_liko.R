...
for(i in 1:100) {
    mydataName <- paste0("~/cloud/project/R9_cal_rand_yps/cal_rand_yps_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

for(i in 1:100) {
    mydataName <- paste0("~/cloud/project/R9_cal_rand_rm/cal_rand_rm_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_cal_rand_yps <- fpo_format(nm="cal_rand_yps_",each_nr = 5,nr = 20, nGenes= 1000,nbit=3)
df_cal_rand_rm <-  fpo_format(nm="cal_rand_rm_",each_nr = 5,nr = 20, nGenes= 1000,nbit=3)
...
ggplot(df_cal_rand_yps, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
...
ggplot(df_cal_rand_rm, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    #geom_abline(slope = 0, intercept = fy1i, color='black',linetype="dashed") +
    #geom_abline(slope = 0, intercept = fy1b, color='black',linetype="dashed") +
    #stat_function(fun=function(x) fy1i + (fy2i-fy1i)*exp(-exp(fy3i) * x ) , geom='line', color="orange", linetype="solid" ) + 
    #stat_function(fun=function(x) fy1b + (fy2b-fy1b)*exp(-exp(fy3b) * x ) , geom='line', color="cyan", linetype="solid" ) + 
    ...
    scale_fill_manual(values=c("steelblue2","tan")) +
    scale_color_manual(values=c( "steelblue2","tan")) +
    labs(title="R_9: DNA_rand_rm_sig0.05", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
    theme(axis.text.x=element_text(angle=0, hjust=1)) +
    coord_cartesian(xlim=c(1,20), ylim=c(0, 0.8)) +
    scale_x_discrete(labels=1:20) +
