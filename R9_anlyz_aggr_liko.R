...
for(i in 1:475) {
    mydataName <- paste0("~/cloud/project/R9_cal_notail_aggr_liko/cal_agrg_liko_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_cal_aggr_notail <- fpo_format(nm="cal_agrg_liko_",each_nr = 25,nr = 19, nGenes= 952,nbit=3)
...
ggplot(df_cal_aggr_notail, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="R_9: DNA_notail_likelihood_ratio_sig0.05", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
