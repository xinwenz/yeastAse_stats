...
for(i in 1:450) {
    mydataName <- paste0("~/cloud/project/R5_exphTrueRM14A9A_liko/exph_trueRM14A9A_liko_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_true <- fpo_format(nm="exph_trueRM14A9A_liko_",each_nr = 25,nr = 18, nGenes= 4710,nbit=3)
...
ggplot(df_true, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="R_5: exph_true_likelihood_ratio_sig0.05", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
