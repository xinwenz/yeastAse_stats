...
df_cal_rand_notail <- fpo_format(nm="cal_rand_liko_",each_nr = 5,nr = 19, nGenes= 1000,nbit=3)
...
ggplot(df_cal_rand_notail, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="R_9: DNA_rand_likelihood_ratio_sig0.05", y="rejection rate", x="level of replication (Nr)") + 
    theme_bw() +
