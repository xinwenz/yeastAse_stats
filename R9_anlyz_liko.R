...
for(i in 1:2850) {
    mydataName <- paste0("~/cloud/project/R9_calh_RM4A_gene_liko/calh_notail_liko_",formatC(i,width=4,flag="0"),".RData")
    load(mydataName)
}

df_calh_notail <- fpo_format(nm="calh_notail_liko_",each_nr = 150,nr = 19, nGenes= 4759,nbit=4)
...
ggplot(df_calh_notail, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
