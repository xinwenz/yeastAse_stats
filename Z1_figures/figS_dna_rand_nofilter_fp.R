...
for(i in 1:95) {
    mydataName <- paste0("~/cloud/project/R9_cal_notail_rand_liko/cal_rand_liko_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_cal_rand<- fpo_format(nm="cal_rand_liko_",each_nr = 5,nr = 19, nGenes= 1000,nbit=3)
...
ggplot(df_cal_rand, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
