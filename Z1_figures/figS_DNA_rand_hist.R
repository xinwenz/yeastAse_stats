load("~/cloud/project/R9_cal_notail_rand_liko/cal_rand_liko_095.RData")

tmpp <- cal_rand_liko_095
#hist(tmpp$log.ecis, breaks=100,xlim=c(-3,3)) ## histgram 
ggplot(tmpp, aes(x=BBf_ec)) + 
    geom_histogram( color="grey60",fill='grey80') + 
    theme_bw() +
    labs(title="hybrid genomic DNA: Distribution of log2(ecis)", y="number of genes", x=expression(paste("log"["2"], "(","e"["cis"],")"))) + 
    theme_bw() +
    theme(axis.text.x=element_text(angle=0, hjust=1)) +
    #coord_cartesian(ylim=c(0, 0.6)) +
    #scale_x_continuous(breaks=seq(-0.3,0.3,by=0.05)) + 
