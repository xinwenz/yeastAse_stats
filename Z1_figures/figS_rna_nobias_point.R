...
x <- colSums(expdc[,grep(".*_yGc",names(expdc),value=T)],na.rm=T)
y <- colSums(expdc[,grep(".*_rGc",names(expdc),value=T)],na.rm=T)
xy <- data.frame(x,y)
ggplot(xy,aes(x=x,y=y)) + 
    geom_point(size=2.2) + 
    labs(title="Expression read counts of 20 replicates using two \n references with filtered positions" , x = "total read counts mapped to YPS128 allele \n (million counts)" , y = "total read counts mapped to RM11-1a allele \n (million counts)" ) + 
    coord_cartesian(ylim=c(2.5*10^6,7.5*10^6),xlim=c(2.5*10^6,7.5*10^6)) +
    scale_y_continuous(labels=seq(2.5,7.5,by=0.5),breaks=seq(2.5*10^6,7.5*10^6,by=0.5*10^6)) +
    #scale_x_continuous(labels = scales::scientific) +
    scale_x_continuous(labels=seq(2.5,7.5,by=0.5),breaks=seq(2.5*10^6,7.5*10^6,by=0.5*10^6)) + 
