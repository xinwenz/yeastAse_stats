...
for(i in 1:3000) {
    mydataName <- paste0("~/cloud/project/R8_calih_gene/calih_gene_",formatC(i,width=4,flag="0"),".RData")
    load(mydataName)
}

df_exph <- fpo_format(nm="calih_gene_",each_nr = 150,nr = 20, nGenes= 4771,nbit=4)


dt_BI <- cbind(1:20,BI_median[1:20])
...
ggplot(df_exph, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="Xinw2018: proportion of cis-affected genes \n discovered with different levels of replication", y="discovery rate", x="level of replication (Nr)") + 
    theme_bw() +
