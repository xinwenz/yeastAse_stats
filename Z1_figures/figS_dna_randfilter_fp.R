...
for(i in 1:95) {
    mydataName <- paste0("~/cloud/project/R92_rand_filter/cal_rand_filter_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_cal_rand_filter <- fpo_format(nm="cal_rand_filter_",each_nr = 5,nr = 19, nGenes= 1000,nbit=3)
...
ggplot(df_cal_rand_filter, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
    ...
    labs(title="Genomic DNA (biasd positions excluded):\n false positive rate with different levels of replication", y="false positive rate", x="level of replication (Nr)") + 
    theme_bw() +
